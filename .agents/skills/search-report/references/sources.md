...
- Vince service: `a479d07f-f963-4ce9-a7db-856e52cb7297`, volume mounted at `/data`, patched v1.11.8 build in `operations/vince/`, start command `/vince serve`.
...
Reports and private strategy notes live under `~/Documents/agent-artifacts/robertjw-discoverability/`. Data collection began September 11, 2026; earlier periods lack coverage. Launch-day visits and contact clicks include setup tests. Contact clicks measure intent, not received emails or successful hires. Stock Vince v1.11.8 maps event-name filters to a nonexistent storage field. The patched analytics build corrects that mapping. The collector uses JSON event filters and reports contact counts only after a nonexistent-event query returns zero for the same dates. An HTTP failure, malformed response or failed control leaves contacts unavailable. The patch also fixes site selection before visitor fields are stored; historical visitor associations lost before the fix are not reconstructed. Read private `validation-activity.json` alongside traffic evidence to distinguish controlled contact tests from real inquiries. Scripted browser checks may be excluded by the tracker; verify actual recorded test events instead of assuming exclusion.
