...
Reports and private strategy notes live under `~/Documents/agent-artifacts/robertjw-discoverability/`. Data collection began September 11, 2026; earlier periods lack coverage. Launch-day visits and contact clicks include setup tests. Contact clicks measure intent, not received emails or successful hires. The patched Vince build in `operations/vince/` fixes event filtering and first-event visitor storage. Contact counts require a zero nonexistent-event control. Historical lost visitor associations are not reconstructed. Read private `validation-activity.json` for recorded test clicks.

Deploy analytics changes with `railway up operations/vince --path-as-root --project 37ec586d-0547-4af4-8c2d-b5787baa0c69 --environment production --service a479d07f-f963-4ce9-a7db-856e52cb7297`; retain its `/data` volume.
